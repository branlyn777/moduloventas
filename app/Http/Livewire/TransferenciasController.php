...
{
    public $nro, $nro_det, $detalle, $estado, $estado_destino, $vs = [],
    $datalist_destino, $selected_id1, $class1, $class, $selected_id2,
    $data_origen, $estado_lista_tr, $estado_lista_te, $estado1, $estado2,
    $mensaje_toast, $selected, $selected2;


    public $dateFrom, $dateTo, $tipo;
    ...
    public function mount()
    {
        ...
        $this->estado1 = 'active show';
        $this->estado2 = 'fade';
    }
    public function cambioestado()
    {
        if($this->estado1 == 'active show')
        {

            $this->estado1 = 'fade';
            $this->estado2 = 'active show';
            $this->selected = 'false';
            $this->selected2 = 'true';
        }
        else
    ...
    public function render()
    {
        ...
        $data_or = Transference::join('estado_transferencias', 'transferences.id', 'estado_transferencias.id_transferencia')
            ...
            ->select(
                'transferences.created_at as fecha_tr',
                'transferences.id as t_id',
                'transferences.estado',
                'users.*',
                'suc_origen.name as origen_name',
                'origen.id as orih',
                'suc_destino.name as destino_name',
                'estado_transferencias.estado as estado_tr',
                'estado_transferencias.op',
                'origen.nombre as origen',
                'destino1.nombre as dst',
                'transferences.created_at as fecha'
            )
        ...
        ->whereBetween('fecha', [$this->dateFrom . ' 00:00:00', $this->dateTo . ' 23:59:59']);
        ...
        $data_destino = Transference::join('estado_transferencias', 'transferences.id', 'estado_transferencias.id_transferencia')
            ...
            ->select(
                'transferences.created_at as fecha_tr',
                'transferences.id as tr_des_id',
                'transferences.estado',
                'users.*',
                'suc_origen.name as origen_name',
                'suc_destino.name as destino_name',
                'estado_transferencias.estado as estado_te',
                'origen.nombre as origen',
                'destino2.nombre as dst2'
            )
            ->whereBetween('transferences.created_at', [$this->dateFrom . ' 00:00:00', $this->dateTo . ' 23:59:59'])
            ...
            ->orderBy('fecha_tr', 'desc')
            ->get();
        return view('livewire.transferencia.verTransferencias', [
            'data_t' => $this->data_origen,
            'data_estado' => $this->estado,
            'data_d' => $data_destino
        ])
    ...
    protected $listeners = [
        'editRow' => 'editar',
        'deleteRow' => 'eliminarTransferencia'
    ];
